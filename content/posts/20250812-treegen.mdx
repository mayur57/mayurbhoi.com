---
title: Introducing Treegen
description: The reverse `tree` command
slug: treegen
uploaded: 2025-08-12
tag: tech
---
...
I set out to **_avoid wasting my time_**.

I found myself, more than once, with a `tree` command output sitting in a text file, describing exactly the layout I wanted somewhere else. Or a README from a tutorial showing me what the scaffold _should_ look like. And each time, I had to translate that into a pile of `mkdir` and `touch` commands.
...
It speaks in the same format we already use to _describe_ structure. The format you find in documentation, blog posts, Stack Overflow answers. The format the `tree` command spits out.
