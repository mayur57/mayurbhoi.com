...
Straightforward enough. The complication: BMTC's concept of "UP" doesn't consistently mean "toward the airport." For most routes it does. For KIA-7, KIA-14, KIA-15A, and KIA-8EW, the UP direction goes _away_ from the airport. So you can't blindly pick the UP file. You have to check which direction's last stop matches the airport name and use that.
...
The cause: Python's `round()` uses **banker's rounding** (round-half-to-even). When a value lands exactly on the 0.5 boundary after scaling, it rounds to the nearest _even_ digit. So `12.976465 * 100000 = 1297646.5` rounds to `1297646` (even). JavaScript's `Math.round()` always rounds 0.5 up, so it gives `1297647`.
...
```jsx
<Polyline positions={route.path} pathOptions={{ color: route.color, weight: 3, opacity: 0.9 }} />
```
